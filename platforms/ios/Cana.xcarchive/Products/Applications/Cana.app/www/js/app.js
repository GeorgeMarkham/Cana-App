    ...
    $('#home_page').on('pageshow', (e) => {
        ...
        if(user){
            ...
            friends.forEach(friend => {
                $('#friends_list_view').append("<li data-name='" + friend + "' id='friend_list_item'>" + friend + "</li>");
            });
...
$('#friend_list_item').on('tap', (e)=>{
    var friend_name = event.target.attributes[1].nodeValue;
...
$('#home_page').on('swipe',( event ) => {
    $('#friends_list_view li').css("background-color", 'blue');
    navigator.notification.alert(
        'Scroll',  // message
        ()=>{
            //do nothing
